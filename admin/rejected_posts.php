...
function getRejectedPosts($conn, $role, $muni_id) {
    if ($role === 'superadmin') {
        ...
        $query = "SELECT p.*, m.MuniName FROM post p 
                  JOIN Municipalities m ON p.muni_id = m.Muni_ID 
                  WHERE p.muni_id = '$muni_id' AND p.status = 'Rejected'";
    }
...
$rejectedPosts = getRejectedPosts($conn, $role, $muni_id);
?>
                ...
                <div class="container-fluid p-0">
                    <div class="card shadow-sm">
                        <div class="card-header bg-danger text-white">
                            <h5 class="card-title mb-0 text-light">Rejected Posts</h5>
                        </div>
                            ...
                            <table class="table table-striped table-hover table-bordered">
                                <thead class="table-dark">
                                    <tr>
                                        ...
                                        <th class="d-none d-md-table-cell">Rejection Reason</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php 
                                        while ($row = mysqli_fetch_assoc($rejectedPosts)) {
                                            echo "<tr>";
                                            ...
                                            echo "<td class='d-none d-md-table-cell'>" . $row['reason_for_rejection'] . "</td>";
                                            echo "</tr>";
                                        }
                                    ?>
                                </tbody>
